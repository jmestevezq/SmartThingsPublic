/**
 *  "Smarter Cube Controller"
 *
 *  For now I just overrided shake and knock events to send a special button 888 and 999
 *  Author: Artur Draga
 *	
 * 	Based on code by: "Xiaomi Magic Cube Controller" Artur Draga
 *
...
metadata {
	definition (name: "Smarterc Cube Controller", namespace: "teakandslate", author: "Jose Estevez") {
		capability "Actuator"
		capability "Button"
...
def Map shakeEvents() {
	//Sending Shake event as special button 999
		sendEvent([ 
			name: "button", 
			value: "pushed", 
			data: [buttonNumber: 999, 
			face: 0], 
			descriptionText: "$device.displayName was shaken.", 
			isStateChange: true])
	/**
...
def Map slideEvents(Integer targetFace) {
    ...
    //Sending Slide event as special button 777
		sendEvent([ 
			name: "button", 
			value: "pushed", 
			data: [buttonNumber: 777, 
			face: targetFace], 
			descriptionText: "$device.displayName was slide with face # $targetFace up.", 
			isStateChange: true])
...
def knockEvents(Integer targetFace) {
	if ( targetFace != device.currentValue("face") as Integer ) { log.info "Stale face data, updating."; setFace(targetFace) }
	//Sending Shake event as special button 888
		sendEvent([ 
			name: "button", 
			value: "pushed", 
			data: [buttonNumber: 888, 
			face: targetFace], 
